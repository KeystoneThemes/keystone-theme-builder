jQuery(window).on("elementor/frontend/init", function () {
    ...
    elementorFrontend.hooks.addAction(
        "frontend/element_ready/stonex-google-map.default",
        function ($scope, $) {

            console.log('ok')
            var mapElement = $scope.find(".stonex_map_height");

            var mapSettings = mapElement.data("settings");

            var mapStyle = mapElement.data("style");

            var stonexMapMarkers = [];

            stonexMap = newMap(mapElement, mapSettings, mapStyle);
            ...
            function newMap(map, settings, mapStyle) {
                ...
                var markers = map.find(".stonex-pin");
            ...
            function add_marker(pin, map, autoOpen, hoverOpen, hoverClose) {
                ...
                stonexMapMarkers.push(marker);

                //Used with Carousel Custom Navigation option
                if (customID) {
                    google.maps.event.addListener(marker, "click", function () {
                        console.log(customID);
                        var $carouselWidget = $(".stonex-carousel-wrapper");
                            ...
                            $carouselWidget.map(function (index, item) {
                                ...
                                if (carouselSettings.navigation) {
                                    if (-1 != carouselSettings.navigation.indexOf("#" + customID)) {
                                        ...
                                        $(item).find(".stonex-carousel-inner").slick("slickGoTo", slideIndex);
                                    }
                                }
                            })
                ...
                if (
                    pin.find(".stonex-maps-info-title").html() ||
                    pin.find(".stonex-maps-info-desc").html()
                ) {
                    // create info window
                            ...
                            if ($carouselWidget.length) {
                                $carouselWidget.map(function (index, item) {
                                    ...
                                    if (carouselSettings.navigation) {
                                        if (-1 != carouselSettings.navigation.indexOf("#" + customID)) {
                                            var slideIndex = carouselSettings.navigation.indexOf("#" + customID);
                                            $carouselWidget.find(".stonex-carousel-inner").slick("slickGoTo", slideIndex);
                                        }
                                    }
                                })
            ...
            if (markerCluster) {
                var markerCluster = new MarkerClusterer(stonexMap, stonexMapMarkers, {
                    imagePath:
