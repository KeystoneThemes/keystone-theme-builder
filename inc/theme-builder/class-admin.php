...
class Admin
{
    ...
    public function __construct()
    {
        ...
        add_filter('views_edit-stonex-tb', [$this, 'admin_print_tabs']);
        add_filter('manage_stonex-tb_posts_columns', [$this, 'custom_columns']);
        add_filter('manage_stonex-tb_posts_custom_column', [$this, 'display_custom_columns']);
        add_action('admin_enqueue_scripts', [$this, 'enqueue_scripts_and_style']);

        //display type in megamenu
        add_filter('wp_setup_nav_menu_item', function ($menu_item) {
            if ($menu_item->object === 'stonex-tb') {
        ...
        add_filter('nav_menu_items_stonex-tb', [$this, 'filter_megamenu_in_menu']);
        add_filter('nav_menu_items_stonex-tb_recent', [$this, 'filter_megamenu_in_menu']);
...
?>
    ...
    {
        $icon_url = get_template_directory_uri() . "/assets/images/theme-options/theme-builder.svg";
        $icon_url = apply_filters('stonex_theme_icon_url', $icon_url);

        $hook = add_menu_page(
            __('Theme Builder', 'stonex'),
            __('Theme Builder', 'stonex'),
            'manage_options',
            'edit.php?post_type=stonex-tb',
            '',
            $icon_url,
            3
        );
    ...
    public function enqueue_scripts_and_style($hook)
    {
        ...
        if ($hook === 'edit.php' && $typenow === 'stonex-tb') {
            // $prefix = (  ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) || defined( 'STONEX_WIDGET' ) ) ? '' : '.min';
            wp_enqueue_script(
                'stonex-tb-manifest',
                STONEX_ASSETS_PUBLIC . '/js/manifest.js',
                [],
                filemtime(STONEX_WIDGET . 'assets/js/manifest.js'),
                true
            );
            wp_enqueue_script(
                'stonex-tb-vendor',
                STONEX_ASSETS_PUBLIC . '/js/vendor.js',
                ['stonex-tb-manifest'],
                filemtime(STONEX_WIDGET . 'assets/js/vendor.js'),
                true
            );
            wp_enqueue_script(
                'stonex-tb',
                STONEX_ASSETS_PUBLIC . '/js/theme-builder.js',
                ['stonex-tb-manifest'],
                filemtime(STONEX_WIDGET . 'assets/js/theme-builder.js'),
                true
            );

            wp_enqueue_style(
                'stonex-tb-styles',
                STONEX_ASSETS_PUBLIC . '/css/theme-builder.css',
                [],
                filemtime(STONEX_WIDGET . 'assets/css/theme-builder.css')
            );

            wp_enqueue_style(
                'stonex-admin',
                STONEX_ASSETS_PUBLIC . '/css/admin.css',
                [],
                filemtime(STONEX_WIDGET . 'assets/css/admin.css')
            );

            $admin_customizer = [];
            $data             = array(
                'conditions'       => \STONEX\Elementor\ThemeBuilder\Common::get_location_selections(),
            ...
            wp_add_inline_script('stonex-tb', 'var stonex_data = ' . json_encode($data), 'before');
        }
    }
    ...
    public function admin_print_tabs($views)
    {
        ...
        $doc_types = array(
            '_type_header' => __('Header', 'stonex'),
            '_type_footer' => __('Footer', 'stonex'),
            '_type_mm'     => __('Mega Menu', 'stonex'),
            '_type_job'    => __('Job', 'stonex'),
            '_type_cs'    => __('Case Study', 'stonex'),
            // '_type_block'  => __( 'Block', 'stonex' ),
            // '_type_popup'  => __( 'Popup', 'stonex' ),
            // '_type_single' => __('Single', 'stonex'),
            // '_type_archive' => __('Archive', 'stonex'),

        );
    ?>

        <div id="stonex-tb-wrapp"></div>
        <div id="stonex-theme-builder-tabs" class="nav-tab-wrapper">
            <a class="nav-tab<?php echo $active_class; ?>" href="<?php echo $baseurl; ?>">
                <?php echo __('All', 'stonex'); ?>
            </a>
            <?php
...
<?php
    ...
    function get_item_info($post_id)
    {
        ...
        if ($type === 'mega menu') {
            ...
            $info = '<input class="wp-ui-text-highlight code" type="text" onfocus="this.select();" readonly="readonly" value="[stonex-block id=&quot;' . $post_id . '&quot;]">';
        } else {
