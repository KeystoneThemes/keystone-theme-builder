...
use STONEX\Settings as Settings;
...
class Common {
    ...
    public function __construct() {
        ...
        add_filter( 'theme_stonex-tb_templates', [$this, 'custom_templates_list'] );
        add_shortcode( 'stonex-block', [$this, 'blocks_shortcode'] );
    }
    ...
    function register_ctp() {
        ...
        register_taxonomy(
            ...
            [
                'hierarchical'      => false,
                'public'            => false,
                'label'             => _x( 'Type', 'Theme Builder', 'stonex' ),
                'show_ui'           => false,
    ...
    function custom_templates( $single ) {
        ...
        if ( $post->post_type == 'stonex-tb' ) {

            //we need more controll on popup
            if ( self::get_the_type( $post->ID ) === 'popup' ) {
                return STONEX_WIDGET_INC . 'theme-builder/templates/popup.php';
            }
            //default
            return STONEX_WIDGET_INC . 'theme-builder/templates/canvas.php';
        }
        return $single;
    }
    ...
    function custom_templates_list( $post_templates ) {
        $post_templates[STONEX_WIDGET_INC . '/elementor/theme-builder/templates/canvas.php'] = "ThemeBuilder Canvas";
        $post_templates[STONEX_WIDGET_INC . '/elementor/theme-builder/templates/popup.php']  = "ThemeBuilder Popup";
    ...
    public static function get_location_selections() {
        ...
        $special_pages = array(
            array(
                'name'  => __( '404 Page', 'stonex' ),
                'value' => 'special-404',
            ),
            array(
                'name'  => __( 'Search Page', 'stonex' ),
                'value' => 'special-search',
            ),
            array(
                'name'  => __( 'Entire Blog', 'stonex' ),
                'value' => 'special-blog',
            ),
            array(
                'name'  => __( 'Front Page', 'stonex' ),
                'value' => 'special-front',
            ),
            array(
                'name'  => __( 'Date Archive', 'stonex' ),
                'value' => 'special-date',
            ),
            array(
                'name'  => __( 'Author Archive', 'stonex' ),
                'value' => 'special-author',
            ),
            array(
                'name'  => __( 'Specific Jobs / Taxonomies, etc', 'stonex' ),
                'value' => 'special_job',
            ),
            array(
                'name'  => __( 'Specific Case Study / Taxonomies, etc', 'stonex' ),
                'value' => 'special_cs',
            ),
        );
        ...
        if ( class_exists( 'WooCommerce' ) ) {
            $special_pages[] = array(
                'name'  => __( 'WooCommerce Shop Page', 'stonex' ),
                'value' => 'special-woo-shop',
            );
            $special_pages[] = array(
                'name'  => __( 'WooCommerce Product Page', 'stonex' ),
        ...
        $selection_options = array(
            'basic'         => array(
                'label' => __( 'Basic', 'stonex' ),
                'value' => array(
                    array(
                        'name'  => __( 'Entire Website', 'stonex' ),
                        'value' => 'basic-global',
                    ),
                    array(
                        'name'  => __( 'All Pages', 'stonex' ),
                        'value' => 'basic-page',
                    ),
                    array(
                        'name'  => __( 'All Blog Posts', 'stonex' ),
                        'value' => 'basic-single',
                    ),
                    array(
                        'name'  => __( 'All Portfolio Posts', 'stonex' ),
                        'value' => 'basic-portfolio',
                    ),
                    array(
                        'name'  => __( 'All Job Posts', 'stonex' ),
                        'value' => 'basic-job',
                    ),
                    array(
                        'name'  => __( 'All Case Study Posts', 'stonex' ),
                        'value' => 'basic-cs',
                    ),
                    array(
                        'name'  => __( 'All Archives', 'stonex' ),
            ...
            'special-pages' => array(
                'label' => __( 'Special Pages', 'stonex' ),
                'value' => $special_pages,
            ),
        );

        $selection_options['specific-target'] = array(
            'label' => __( 'Specific Target', 'stonex' ),
            'value' => array(
                array(
                    'name'  => __( 'Specific Pages / Posts / Taxonomies, etc.', 'stonex' ),
                    'value' => 'specifics',
                ),
            ),
        );

        // $selection_options['job_basic'] = [
        //     'job-basic' => array(
        //         'label' => __( 'Job Posts', 'stonex' ),
        //         'value' => array(
        //             array(
        //                 'name'  => __( 'Entire Job Posts', 'stonex' ),
        //                 'value' => 'job-global',
        //             ),
        //             array(
        //                 'name'  => __( 'Specific Jobs / Taxonomies, etc.', 'stonex' ),
...
if ( $trigger && !$is_editor ) {
    ...
    static function get_js( $id, $trigger, $settings ) {
        ...
        if ( $trigger['maxShow']['enable'] === 'true' ) {
            $js .= "
            if(!localStorage.getItem('stonex_popup_" . $id . "') || (localStorage.getItem('stonex_popup_" . $id . "') < " . $trigger['maxShow']['amount'] . " ) ){
                ";

            $extra .= "
            localStorage.setItem('stonex_popup_" . $id . "', Number(localStorage.getItem('stonex_popup_" . $id . "')) + 1); ";
        }
    ...
    static function get_generic_style() {
        ...
        if ( !$is_embeded ) {
            ...
            <style id="ui-popup-style">
                ...
                .ui-popup-wrapper [data-elementor-type=stonex-tb] {
                    overflow: hidden auto;
                    width: 100%;
                }

                .ui-popup-wrapper [data-elementor-type=stonex-tb] .elementor-section-wrap:not(:empty)+#elementor-add-new-section {
                    display: none;
                }
